<!--
  DRAFT for the rewritten README, targeting pyOpenSci resubmission.
  Compare against the current README.md before swapping.
  Opening rewritten; reference sections preserved verbatim from README.md.
-->
...
# 2. Drill in on anything flagged (returns pandas DataFrames)
...
mismap-qc handles all three with a single API and readers for AnnData and
common proteomics formats.
...
<!--
  Everything below this point is preserved verbatim from the current README.md.
  These are the reference docs for individual functions; the rewrite only
  changes the opening framing and ordering.
-->
